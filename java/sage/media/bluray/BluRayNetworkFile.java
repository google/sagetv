 ...
 * @author Narflex
 */
public class BluRayNetworkFile extends sage.NetworkChannelFile implements BluRayStreamer
{

	/** Creates a new instance of BluRayNetworkFile */
	public BluRayNetworkFile(String hostname, java.io.File bdmvDir, boolean directBuffer, int inTargetTitle) throws java.io.IOException
    {
		super(hostname, (String)null, "r", sage.media.bluray.BluRayParser.BLURAY_CHARSET, directBuffer);
		...
		for (int i = 0; i < fileSequence.length; i++)
		{
			...
			remoteFilename = fileSequence[i].getAbsolutePath();
			openConnection();
			totalSize += getRemoteSize();
			closeConnection();
		...
		currFileIndex = 0;
		remoteFilename = fileSequence[currFileIndex].getAbsolutePath();
		openConnection();
	...
	protected void ensureProperFile(boolean alwaysSeek, boolean forceOpen) throws java.io.IOException
	{
		...
		if (forceOpen || fp < fileOffsets[currFileIndex] || (currFileIndex < fileOffsets.length - 1 && fp >= fileOffsets[currFileIndex + 1]))
		{
			...
			remoteFilename = new java.io.File(streamDir, currPlaylist.playlistItems[currFileIndex].itemClips[currAngle].clipName +
	...
	{
		return fp - fileOffsets[currFileIndex];
	}
	...
	public void read(java.nio.ByteBuffer b) throws java.io.IOException
	{
		...
		{
			ensureProperFile(false, false);
			int currRead = readFromServer(leftToRead, b);
			if (currRead < 0)
				throw new java.io.EOFException();
			fp += currRead;
			leftToRead -= currRead;
