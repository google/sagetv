...
static int InterpretProfileLevel( H264_VIDEO_I *pH264Video )
{
	//int ret = 1; 
...
int ReadH264VideoHeader( H264_VIDEO_I *pH264Video, const unsigned char* pData, int Size )
{
			...
			if ( nal_unit_type == NAL_SPS )
			{
				...
				}
				{
					...
					{                                                                          
						...
						ReadUE( &bits ); //int frame_cropping_rect_right_offset     = ReadUE( &bits );
						ReadUE( &bits ); //int frame_cropping_rect_top_offset       = ReadUE( &bits );
						ReadUE( &bits ); //int frame_cropping_rect_bottom_offset    = ReadUE( &bits );
					}                                                                          
					...
					if( ReadBitsU( &bits, 1 ) )
					{
						...
						{
							pH264Video->frame_rate_nomi = time_scale;
							pH264Video->frame_rate_deno = 2*num_units_in_tick;
						}
					}
...
/////////////////////////////////////////////////////////////
	...
	{
		if ( InterpretProfileLevel( pH264Video ) >  0  )
		{
			return 1;
		} else
		{
