...
public:
	CPlayerData() : m_pSrcFilter(0), m_pDeMux(0), m_pVideoDecoder(0),
		...
		m_pPluginHandler(NULL), m_ccDisabled(0),
#ifdef ENABLE_SAGETV_WMDRM_SUPPORT
	...
	virtual HRESULT SetSourceFilename(const WCHAR** pwFilename, DWORD dwNumFiles, const char* szHostname);
	...
	void CreatePluginHandler();
	GraphPluginHandler* GetGraphPluginHandler() { return m_pPluginHandler; }
	void CallPluginGraphStopped();
	...
	void DisableCC() { m_ccDisabled = TRUE; }
...
protected:
	...
	GraphPluginHandler* m_pPluginHandler;

	static long EventDump( void* context, short bytes, void* mesg );
#ifdef ENABLE_SAGETV_WMDRM_SUPPORT
	CKeyProvider* prov;
#endif
};