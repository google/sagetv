...
HRESULT CPlayerData::SetSourceFilename(const WCHAR* pwFilename, const char* szHostname)
{
	...
	else
	{
			...
			"EnableStandardFilePlayback", 1) && !RequiresCustomSourceFilter())
		{
			if (szHostname && strlen(szHostname))
			{
				...
				HANDLE hFind;
				hFind = FindFirstFileW(pwFilename, &FindFileData);
		...
		else
		{
			...
			if (wlen > 4)
			{
				if (!_wcsicmp(&pwFilename[wlen - 4], L".wmv") ||
					!_wcsicmp(&pwFilename[wlen - 4], L".wma") ||
					!_wcsicmp(&pwFilename[wlen - 4], L".asf"))
				{
					m_requiredWMSource = TRUE;
				}
				if (wlen > 7 && !_wcsicmp(&pwFilename[wlen - 7], L".dvr-ms"))
				{
					requiredDShowSource = TRUE;
				}
				// NOTE: Disable our source/demux for .m2ts as well until we have our demux supporting it properly
				if (wlen >= 5 && (!_wcsicmp(&pwFilename[wlen - 5], L".tivo") || !_wcsicmp(&pwFilename[wlen - 5], L".flac") ||
					!_wcsicmp(&pwFilename[wlen-5], L".m2ts") || !_wcsicmp(&pwFilename[wlen-4], L".mkv")))
				{
					requiredDShowSource = TRUE;
				}
			}
			...
			{
				usedSageTVSource = FALSE;
				hr = m_pGraph->AddSourceFilter(pwFilename, pwFilename, &m_pSrcFilter);
				if (FAILED(hr)) return hr;
	...
	if (fileNeedsLoading)
	{
		...
		/*if (useMyMux)
		{
			...
			hr = pfsf->Load(wsz, &cmt);
		}
		else*/
		{
			hr = pfsf->Load(pwFilename, pmt);
		}
		pfsf->Release();
		if (FAILED(hr)) return hr;
	}
	...
	if ((mpegOK || mcMpegOK) && !requiredDShowSource)
	{
...
HRESULT CPlayerData::RenderAudio()
{
	...
	while (1)
	{
		...
		if (audioSource)
		{
			...
			{
				int currHasAC3 = (pMT->subtype == MEDIASUBTYPE_DOLBY_AC3);
				int currChans = 0;
				if (pMT->formattype == FORMAT_WaveFormatEx)
				{
					currChans = ((WAVEFORMATEX*)pMT->pbFormat)->nChannels;
				}
				if (bestStreamNum == -1 || (currHasAC3 && (!bestStreamHasAC3 || (currChans > bestStreamChannels))))
				{
					bestStreamNum = audioStreamNum;
					bestStreamHasAC3 = currHasAC3;
...
/*				// Check for EVR video processing settings
				...
				if (SUCCEEDED(hr))
				{
					...
					if (SUCCEEDED(hr))
					{
						...
						if (numProcModes)
						{
							hr = pVidProcEvr->SetVideoProcessorMode(&procModeGuids[0]);
							slog(("EVR set the video processing mode! hr=0x%x\r\n", hr));
							HTESTPRINT(hr);
						}
...
{
	if (m_bUnifiedDecoder) return S_OK;
	HRESULT hr = S_OK;
