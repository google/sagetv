...
Param::Param(std::string _name) :
	...
	engine_val = new mathtype();

	default_init_val.mathtype_val = DEFAULT_FLOAT_IV;
        upper_bound.mathtype_val = DEFAULT_FLOAT_UB;
        lower_bound.mathtype_val = DEFAULT_FLOAT_LB;

    /// @note may have fixed a recent bug. testing
    *((mathtype*)engine_val) = default_init_val.mathtype_val;
...
Param * Param::new_param_mathtype(const char * name, short int flags, void * engine_val, void * matrix,
                               mathtype upper_bound, mathtype lower_bound, mathtype init_val) {
    ...
    if ((param = new Param(name, P_TYPE_FLOAT, flags, engine_val, matrix,iv, ub, lb)) == NULL)
        return NULL;
