...
mathtype GenExpr::eval_gen_expr ( int mesh_i, int mesh_j )
{
	mathtype l;
...
mathtype PrefunExpr::eval_prefun_expr ( int mesh_i, int mesh_j )
{


	assert ( func_ptr );

	mathtype * arg_list = new mathtype[this->num_args];

	assert(arg_list);

#ifdef EVAL_DEBUG_FLOAT
	DWRITE ( "fn[" );
#endif
	...
	/* Now we call the function, passing a list of
	   mathtypes as its argument */

	const mathtype value = ( func_ptr ) ( arg_list );
...
mathtype ValExpr::eval_val_expr ( int mesh_i, int mesh_j )
{


	/* Value is a constant, return the mathtype value */
	if ( type == CONSTANT_TERM_T )
	{
#ifdef EVAL_DEBUG
		DWRITE ( "%.4f", mathtofloat(term.constant) );
#endif
	...
	if ( type == PARAM_TERM_T )
	{
		switch ( term.param->type )
		{
			...
			case P_TYPE_BOOL:
#ifdef EVAL_DEBUG
				DWRITE ( "(%s:%.4f)", term.param->name.c_str(), ( mathtype ) ( * ( ( bool* ) ( term.param->engine_val ) ) ) );
#endif


				return ( mathtype ) ( * ( ( bool* ) ( term.param->engine_val ) ) ) ? mathone : mathzero;
			case P_TYPE_INT:
#ifdef EVAL_DEBUG
				...
				return ( mathtype ) mathval(( * ( ( int* ) ( term.param->engine_val ) ) ));
			case P_TYPE_FLOAT:
				...
				if ( term.param->matrix_flag | ( term.param->flags & P_FLAG_ALWAYS_MATRIX ) )
				{
					...
					if ( mesh_i >= 0 )
					{
						...
						{
#ifdef EVAL_DEBUG
				DWRITE ( "(%s:%.4f)", term.param->name.c_str(), mathtofloat(( ( ( mathtype** ) term.param->matrix ) [mesh_i][mesh_j] )));
#endif
							return ( ( ( mathtype** ) term.param->matrix ) [mesh_i][mesh_j] );
						}
						else
						{
#ifdef EVAL_DEBUG
				DWRITE ( "(%s:%.4f)", term.param->name.c_str(), mathtofloat( ( ( mathtype* ) term.param->matrix ) [mesh_i]));
#endif
							return ( ( ( mathtype* ) term.param->matrix ) [mesh_i] );
						}
					}
					//assert(mesh_i >=0);
				}
				//std::cout << term.param->name << ": " << (*((mathtype*)term.param->engine_val)) << std::endl;
#ifdef EVAL_DEBUG
				DWRITE ( "(%s:%.4f)", term.param->name.c_str(), mathtofloat(* ( ( mathtype* ) ( term.param->engine_val ) ) ));
#endif
				return * ( ( mathtype* ) ( term.param->engine_val ) );			
			default:
							DWRITE ( "error in eval_val_expr");
				return EVAL_ERROR;
		}
	}
...
mathtype TreeExpr::eval_tree_expr ( int mesh_i, int mesh_j )
{

	mathtype left_arg, right_arg;
	...
	switch ( infix_op->type )
	{
		...
		case INFIX_MINUS:
			return ( left_arg - right_arg );
		case INFIX_MULT:
			return ( mathmul(left_arg,right_arg) );
		case INFIX_MOD:
			if ( ( int ) right_arg == 0 )
			{
#ifdef EVAL_DEBUG
			...
			return mathval( mathtoint(left_arg) % mathtoint(right_arg) );
		case INFIX_OR:
			return mathval( ( int ) mathtoint(left_arg) | ( int ) mathtoint(right_arg) );
		case INFIX_AND:
			return mathval( ( int ) mathtoint(left_arg) & ( int ) mathtoint(right_arg) );
		case INFIX_DIV:
			if ( right_arg == 0 )
			...
			return ( mathdiv(left_arg,right_arg) );
		default:
#ifdef EVAL_DEBUG
...
/* Converts a mathtype value to a general expression */
GenExpr * GenExpr::const_to_expr ( mathtype val )
{
...
GenExpr * GenExpr::prefun_to_expr ( mathtype ( *func_ptr ) ( void * ), GenExpr ** expr_list, int num_args )
{
	...
	prefun_expr->func_ptr = ( mathtype ( * ) ( void* ) ) func_ptr;
