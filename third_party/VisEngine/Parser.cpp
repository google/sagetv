...
GenExpr * Parser::parse_gen_expr ( std::istream &  fs, TreeExpr * tree_expr, Preset * preset)
{
  ...
  case tLPr:
    ...
    if ((func = BuiltinFuncs::find_func(string)) != NULL)
    {
      ...
      if ((gen_expr = GenExpr::prefun_to_expr((mathtype (*)(void *))func->func_ptr, expr_list, func->getNumArgs())) == NULL)
      {
  ...
  default:
    ...
    /* CASE 1: Check if string is a just a mathtypeing point number */
    if (string_to_mathtype(string, &val) != PROJECTM_PARSE_ERROR)
    {
...
/* Parses a mathtypeing point number */
int Parser::string_to_mathtype(char * string, mathtype * mathtype_ptr)
{
  ...
  double d = strtod(string, error_ptr);
  (*mathtype_ptr) = mathval(d);
...
/* Parses a mathtypeing point number */
int Parser::parse_mathtype(std::istream &  fs, mathtype * mathtype_ptr)
{
  ...
  if (PARSE_DEBUG) printf("parse_mathtype: mathtype conversion failed for string \"%s\"\n", string);
...
InitCond * Parser::parse_init_cond(std::istream &  fs, char * name, Preset * preset)
{
  ...
  /* mathtype value */
  else if (param->type == P_TYPE_FLOAT)
  {
    if ((parse_mathtype(fs, (mathtype*)&init_val.mathtype_val)) == PROJECTM_PARSE_ERROR)
    {
      if (PARSE_DEBUG) printf("parse_init_cond: error parsing mathtype!\n");
      return NULL;
    }
  }
...
int Parser::parse_wavecode(char * token, std::istream &  fs, Preset * preset)
{
  ...
  else if (param->type == P_TYPE_FLOAT)
  {
    if ((parse_mathtype(fs, (mathtype*)&init_val.mathtype_val)) == PROJECTM_PARSE_ERROR)
    {
      if (PARSE_DEBUG) printf("parse_wavecode: error parsing mathtype!\n");
      return PROJECTM_PARSE_ERROR;
    }
  }
...
int Parser::parse_shapecode(char * token, std::istream &  fs, Preset * preset)
{
  ...
  else if (param->type == P_TYPE_FLOAT)
  {
    if ((parse_mathtype(fs, (mathtype*)&init_val.mathtype_val)) == PROJECTM_PARSE_ERROR)
    {
      if (PARSE_DEBUG) printf("parse_shapecode: error parsing mathtype!\n");
      return PROJECTM_PARSE_ERROR;
    }
  }
