...
param->matrix_flag = false;
  ...
  /* Parameter is of a mathtype type, just like C */

  if (param->type == P_TYPE_FLOAT) {
	assert(param->engine_val);
	*((mathtype*)param->engine_val) = init_val.mathtype_val;
    return;
  }
...
void InitCond::init_cond_to_string() {
	...
	switch (param->type) {
		...
		case P_TYPE_INT:
			...
			break;
		case P_TYPE_FLOAT:
			sprintf(string, "%s=%f\n", param->name.c_str(), mathtofloat(init_val.mathtype_val));
			break;
		default:
			return;
	}
