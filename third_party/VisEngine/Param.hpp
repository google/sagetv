 ...
 */
/**
 * $Id: Param.hpp,v 1.1 2009-09-17 14:26:03 jeanfrancois Exp $
 *
...
public:
    ...
    short int type; /* parameter number type (int, bool, or mathtype) */	
    ...
    /// Create a user defined mathtypeing point parameter
    Param( std::string name );

    static bool is_valid_param_string( const char *string );
    void set_param( mathtype val );

    static Param *new_param_mathtype( const char *name, short int flags, void *engine_val,
                             void *matrix, mathtype upper_bound, 
                             mathtype lower_bound,
                             mathtype init_val );
...
inline void Param::set_param( mathtype val) {
    ...
    case P_TYPE_FLOAT:
        /* Make sure value is an integer */
        if (val < lower_bound.mathtype_val)
            *((mathtype*)engine_val) = lower_bound.mathtype_val;
        else if (val > upper_bound.mathtype_val)
            *((mathtype*)engine_val) = upper_bound.mathtype_val;
        else
            *((mathtype*)engine_val) = val;
        break;
    default:
	abort();
        break;
