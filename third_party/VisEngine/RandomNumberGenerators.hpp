...
/* Uniform random number generator x(n+1)= a*x(n) mod c
				...
				Copyright (c) Tao Pang 1997. */
	{
		...
		rc = ic;
		return mathval(iseed/rc);
	}

inline mathtype gaussian(mathtype mean, mathtype sigma)
{

	float x1, x2, w, y1, y2;
 
	do {
		x1 = 2.0f * mathtofloat(uniform()) - 1.0f;
		x2 = 2.0f * mathtofloat(uniform()) - 1.0f;
		w = x1 * x1 + x2 * x2;
	} while ( w >= 1.0 );
	...
	float ret = y1*mathtofloat(sigma) + mathtofloat(mean);
...
inline std::size_t weightedRandomNormalized(std::vector<mathtype> weights) {

#ifdef WIN32	
    // Choose a random bounded mass between 0 and 1
	mathtype cutoff = ((mathtype)(rand())) / RAND_MAX;
#endif 

#ifdef LINUX
    // Choose a random bounded mass between 0 and 1
	mathtype cutoff = ((mathtype)(random())) / RAND_MAX;
#endif
