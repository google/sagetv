 ...
 */
/**
 * $Id: PCM.cpp,v 1.2 2010-03-06 00:20:59 jeanfrancois Exp $
 *
...
void PCM::initPCM(int samples) {
  ...
  //Allocate memory for PCM data buffer
    assert(samples == 2048);
  PCMd = (mathtype **)wipemalloc(2 * sizeof(mathtype *));
  PCMd[0] = (mathtype *)wipemalloc(samples * sizeof(mathtype));
  PCMd[1] = (mathtype *)wipemalloc(samples * sizeof(mathtype));
  ...
  w=  (mathtype *)wipemalloc(maxsamples*sizeof(mathtype));
...
//    this->pcmdataR = NULL;
    ...
    pcmdataL=(mathtype *)wipemalloc(this->maxsamples*sizeof(mathtype));
    pcmdataR=(mathtype *)wipemalloc(this->maxsamples*sizeof(mathtype));
...
void PCM::addPCMmathtype(const mathtype *PCMdata, int samples) const
{
  int i,j;
...
void PCM::addPCM16Data(const short* pcm_data, short samples) const {
   ...
   for (i = 0; i < samples; ++i) {
      j=i+start;
      PCMd[0][j % maxsamples]=mathdiv(mathval(pcm_data[i * 2 + 0]),mathval(16384.0f));
      PCMd[1][j % maxsamples]=mathdiv(mathval(pcm_data[i * 2 + 1]),mathval(16384.0f));
   }
	 ...
	 for(i=0;i<samples;i++)
	   {
         ...
         if ( PCMdata[0][i] != 0 && PCMdata[1][i] != 0 ) {
	         PCMd[0][j%maxsamples]=mathdiv(mathval(PCMdata[0][i]),mathval(16384.0f));
	         PCMd[1][j%maxsamples]=mathdiv(mathval(PCMdata[1][i]),mathval(16384.0f));
          } else {
             PCMd[0][j % maxsamples] = (mathtype)0;
             PCMd[1][j % maxsamples] = (mathtype)0;
          }
	   }
	 ...
	 for(i=0;i<samples;i++)
	   {
	     j=i+start;
         if ( PCMdata[0][i] != 0 && PCMdata[1][i] != 0 ) {
	         PCMd[0][j%maxsamples]=( (mathtype)( PCMdata[0][i] - 128.0 ) / 64 );
	         PCMd[1][j%maxsamples]=( (mathtype)( PCMdata[1][i] - 128.0 ) / 64 );  
          } else {
             PCMd[0][j % maxsamples] = 0;
             PCMd[1][j % maxsamples] = 0;
          }
...
void PCM::getPCM(mathtype *PCMdata, int samples, int channel, int freq, mathtype smoothing, int derive) const
{
   int i,index;
...
/*   if (freq) 
     ...
     {
       mathtype temppcm[1024];
       for (int i=0;i<samples;i++)
	 {temppcm[i]=(mathtype)PCMdata[i];}
       rdft(samples, 1, temppcm, ip, w);
       for (int j=0;j<samples;j++)
	 {PCMdata[j]=(mathtype)temppcm[j];}
     }*/
}
...
int PCM::getPCMnew(mathtype *PCMdata, int channel, int freq, mathtype smoothing, int derive, int reset) const
{
   int i,index;
   ...
   for(i=1;i<newsamples;i++)
     {
       ...
       PCMdata[i]=mathmul((mathval(1)-smoothing),PCMd[channel][index])+mathmul(smoothing,PCMdata[i-1]);
     }
