 ...
 * - Moved to new package sage.media.exif.metadata.exif
...
/**
 * Decodes Exif binary data, populating a <code>Metadata</code> object with
 * tag values in <code>ExifDirectory</code>, <code>GpsDirectory</code> and
 * one of the many camera makernote directories.
 * 
...
public class ExifReader implements MetadataReader {
     ...
     */
    private static final int[] BYTES_PER_FORMAT = { 0, 1, 1, 2, 4, 8, 1, 1, 2,
            4, 8, 4, 8 };
    ...
    // Format types
    // Note: Cannot use the DataFormat enumeration in the case statement that
    // uses these tags.
     ...
     * 
     * @param segmentData
     * @deprecated Not all files will be Jpegs! This overload doesn't offer much
     *             convenience to the caller.
     */
    public ExifReader(JpegSegmentData segmentData) {
        // TODO consider removing this constructor and requiring callers to pass
        // a byte[] or other means to read the exif segment in isolation... not
        // all files will be Jpegs!
     ...
     * @deprecated Not all files will be Jpegs! Use a constructor that provides
    ...
    public ExifReader(InputStream jpegInputStream)
        ...
        // a byte[] or other means to read the exif segment in isolation... not
        // all files will be Jpegs!
        this(new JpegSegmentReader(jpegInputStream)
                .readSegment(JpegSegmentReader.SEGMENT_APP1));
    }
    ...
    public Metadata extract(Metadata metadata) {
        ...
        if (_data.length <= 14) {
            directory
                    .addError("Exif data segment must contain at least 14 bytes");
            return _metadata;
        }

        if (!isExifSegment(_data)) {
    ...
    public static boolean isExifSegment(byte[] data) {
    ...
    private ExifDirectory getExifDirectory() {
        if (_exifDirectory == null) {
            _exifDirectory = (ExifDirectory) _metadata
    ...
    /**
     * Performs the Exif data extraction on a Tiff/Raw, adding found values to
     * the specified instance of <code>Metadata</code>.
     */
    ...
    private Metadata extractIFD(Metadata metadata, int tiffHeaderOffset) {
        ...
        if (!setByteOrder(byteOrderIdentifier)) {
            directory
                    .addError("Unclear distinction between Motorola/Intel byte ordering: "
                            + byteOrderIdentifier);
            return _metadata;
        }
        ...
        if (get16Bits(2 + tiffHeaderOffset) != 0x2a) {
            directory
                    .addError("Invalid Exif start - should have 0x2A at offset 8 in Exif header");
            return _metadata;
        }

        int firstDirectoryOffset = get32Bits(4 + tiffHeaderOffset)
                + tiffHeaderOffset;

        // David Ekholm sent an digital camera image that has this problem
        if (firstDirectoryOffset >= _data.length - 1) {
            directory
                    .addError("First exif directory offset is beyond end of Exif data segment");
            // First directory normally starts 14 bytes in -- try it here and
            // catch another error in the worst case
        ...
        processDirectory(directory, processedDirectoryOffsets,
                firstDirectoryOffset, tiffHeaderOffset,0);

        // after the extraction process, if we have the correct tags, we may be
        // able to store thumbnail information
    ...
    private void storeThumbnailBytes(ExifDirectory exifDirectory,
        ...
        if (!exifDirectory.containsTag(ExifDirectory.TAG_THUMBNAIL_LENGTH)
                || !exifDirectory
                        .containsTag(ExifDirectory.TAG_THUMBNAIL_OFFSET))
            return;

        try {
            int offset = exifDirectory
                    .getInt(ExifDirectory.TAG_THUMBNAIL_OFFSET);
            int length = exifDirectory
                    .getInt(ExifDirectory.TAG_THUMBNAIL_LENGTH);
            byte[] result = new byte[length];
            for (int i = 0; i < result.length; i++) {
                result[i] = _data[tiffHeaderOffset + offset + i];
            }
            exifDirectory
                    .setByteArray(ExifDirectory.TAG_THUMBNAIL_DATA, result);
        } catch (Throwable e) {
            exifDirectory.addError("Unable to extract thumbnail: "
                    + e.getMessage());
        }
    }

    /**
     * Sets the _isMotorollaByteOrder flag to true or false, depending upon the
     * file's byte order string. If the string cannot be interpreted, false is
     * returned.
     * 
     * @param byteOrderIdentifier
     *            a two-character string; either "MM" for Motorolla or "II" for
     *            Intel.
    ...
    /**
     * Process one of the nested Tiff IFD directories. 2 bytes: number of tags
     * for each tag 2 bytes: tag type 2 bytes: format code 4 bytes: component
     * count
     */
    private void processDirectory(Directory directory,
            HashMap processedDirectoryOffsets, int dirStartOffset,
            int tiffHeaderOffset, int offsetSchema) {
    	
    	List makerNoteOffsets=new LinkedList();
    	

    	boolean switchedByteOrder=false;
    	
    	do {
    		
	        // check for directories we've already visited to avoid stack overflows
	        // when recursive/cyclic directory structures exist
	        if (processedDirectoryOffsets.containsKey(new Integer(dirStartOffset)))
	            break;
	
	        // remember that we've visited this directory so that we don't visit it
	        // again later
	        processedDirectoryOffsets.put(new Integer(dirStartOffset), "processed");
	
	        if (dirStartOffset >= _data.length || dirStartOffset < 0) {
	            directory
	                    .addError("Ignored directory marked to start outside data segement");
	            break;
	        }
	
	        if (! isDirectoryLengthValid(dirStartOffset, tiffHeaderOffset)) {
	            /** 
	             * some tools (windows vista) modify the byte order for the EXIF data,
	             * but copy the MakerNote bytes without switching byte order.
	             * This flag temporarily switches byte order while processing a makernote dir
	             */
	        	_isMotorollaByteOrder=!_isMotorollaByteOrder;
	        	switchedByteOrder=true;
	        	if (! isDirectoryLengthValid(dirStartOffset, tiffHeaderOffset)) {
	        ...
	        for (int tagNumber = 0; tagNumber < dirTagCount; tagNumber++) {
	            ...
	            // each component may have more than one byte... calculate the total
	            // number of bytes
	            final int byteCount = componentCount * BYTES_PER_FORMAT[formatCode];
	            final int tagValueOffset = calculateTagValueOffset(byteCount,
	                    tagOffset, tiffHeaderOffset,offsetSchema);
	            if (tagValueOffset < 0 || tagValueOffset > _data.length) {
	                directory.addError("Illegal pointer offset value in EXIF");
	                continue;
	            }
	
	            // Check that this tag isn't going to allocate outside the bounds of
	            // the data array.
	            ...
	            // Calculate the value as an offset for cases where the tag
	            ...
	            case TAG_EXIF_OFFSET:
	                processDirectory(_metadata.getDirectory(ExifDirectory.class),
	                        processedDirectoryOffsets, subdirOffset,
	                        tiffHeaderOffset,offsetSchema);
	                continue;
	            case TAG_INTEROP_OFFSET:
	                processDirectory(_metadata
	                        .getDirectory(ExifInteropDirectory.class),
	                        processedDirectoryOffsets, subdirOffset,
	                        tiffHeaderOffset,offsetSchema);
	                continue;
	            case TAG_GPS_INFO_OFFSET:
	                processDirectory(_metadata.getDirectory(GpsDirectory.class),
	                        processedDirectoryOffsets, subdirOffset,
	                        tiffHeaderOffset,offsetSchema);
	                continue;
	            case TAG_MAKER_NOTE:
	            	makerNoteOffsets.add(new Integer(tagValueOffset));
	                continue;
	            default:
	                processTag(directory, tagType, tagValueOffset, componentCount,
	                        formatCode);
	                break;
	            }
	        }
	        ...
	        final int finalTagOffset = calculateTagOffset(dirStartOffset,
	                dirTagCount);
	        ...
	        if (nextDirectoryOffset != 0) {
	            ...
	            if (nextDirectoryOffset >= _data.length) {
	                // Last 4 bytes of IFD reference another IFD with an address
	                // that is out of bounds
	                // Note this could have been caused by jhead 1.3 cropping too
	                // much
	                break;
	            } else if (nextDirectoryOffset < dirStartOffset) {
	                // Last 4 bytes of IFD reference another IFD with an address
	                // that is before the start of this directory
	                break;
	            }
	            // the next directory is of same type as this one
	        }
	        dirStartOffset=nextDirectoryOffset;
    	} while ( dirStartOffset!=0 );

    	// process makerNotes
    	if ( offsetSchema==0){
	    	try {
	    		offsetSchema=directory.getInt(ExifDirectory.TAG_OFFSET_SCHEMA);
	    	} catch (MetadataException e){
	    		offsetSchema=0;
	    	}
    	}
    	
    	for (Iterator it = makerNoteOffsets.iterator(); it
				.hasNext();) {
			Integer offset = (Integer) it.next();
	        processMakerNote(offset.intValue(), processedDirectoryOffsets,
	                tiffHeaderOffset,offsetSchema);
		}
    	
    	if ( switchedByteOrder)
    		_isMotorollaByteOrder=!_isMotorollaByteOrder;
    }

    private void processMakerNote(int subdirOffset,
            HashMap processedDirectoryOffsets, int tiffHeaderOffset, int offsetSchema) {
        ...
        if ("OLYMP".equals(firstFiveChars) || "EPSON".equals(firstFiveChars)
                || "AGFA".equals(firstFourChars)) {
            // Olympus Makernote
            // Epson and Agfa use Olypus maker note standard, see:
            // http://www.ozhiker.com/electronics/pjmt/jpeg_info/
            processDirectory(_metadata
                    .getDirectory(OlympusMakernoteDirectory.class),
                    processedDirectoryOffsets, subdirOffset + 8,
                    tiffHeaderOffset,offsetSchema);
        } else if (cameraModel != null
                && cameraModel.trim().toUpperCase().startsWith("NIKON")) {
            if ("Nikon".equals(firstFiveChars)) {
                ...
                if (_data[subdirOffset + 6] == 1)
                    processDirectory(_metadata
                            .getDirectory(NikonType1MakernoteDirectory.class),
                            processedDirectoryOffsets, subdirOffset + 8,
                            tiffHeaderOffset,offsetSchema);
                else if (_data[subdirOffset + 6] == 2)
                    processDirectory(_metadata
                            .getDirectory(NikonType2MakernoteDirectory.class),
                            processedDirectoryOffsets, subdirOffset + 18,
                            subdirOffset + 10,offsetSchema);
                else
                    exifDirectory
                            .addError("Unsupported makernote data ignored.");
            } else {
                // The IFD begins with the first MakerNote byte (no ASCII name).
                // This occurs with CoolPix 775, E990 and D1 models.
                processDirectory(_metadata
                        .getDirectory(NikonType2MakernoteDirectory.class),
                        processedDirectoryOffsets, subdirOffset,
                        tiffHeaderOffset,offsetSchema);
            }
        } else if ("SONY CAM".equals(firstEightChars)
                || "SONY DSC".equals(firstEightChars)) {
            processDirectory(_metadata
                    .getDirectory(SonyMakernoteDirectory.class),
                    processedDirectoryOffsets, subdirOffset + 12,
                    tiffHeaderOffset,offsetSchema);
        } else if ("KDK".equals(firstThreeChars)) {
            processDirectory(_metadata
                    .getDirectory(KodakMakernoteDirectory.class),
                    processedDirectoryOffsets, subdirOffset + 20,
                    tiffHeaderOffset,offsetSchema);
        } else if ("Canon".equalsIgnoreCase(cameraModel)) {
            processDirectory(_metadata
                    .getDirectory(CanonMakernoteDirectory.class),
                    processedDirectoryOffsets, subdirOffset, tiffHeaderOffset,offsetSchema);
        } else if (cameraModel != null
                && cameraModel.toUpperCase().startsWith("CASIO")) {
            if ("QVC\u0000\u0000\u0000".equals(firstSixChars))
                processDirectory(_metadata
                        .getDirectory(CasioType2MakernoteDirectory.class),
                        processedDirectoryOffsets, subdirOffset + 6,
                        tiffHeaderOffset,offsetSchema);
            else
                processDirectory(_metadata
                        .getDirectory(CasioType1MakernoteDirectory.class),
                        processedDirectoryOffsets, subdirOffset,
                        tiffHeaderOffset,offsetSchema);
        } else if ("FUJIFILM".equals(firstEightChars)
                || "Fujifilm".equalsIgnoreCase(cameraModel)) {
            // TODO make this field a passed parameter, to avoid threading
            // issues
            boolean byteOrderBefore = _isMotorollaByteOrder;
            // bug in fujifilm makernote ifd means we temporarily use Intel byte
            // ordering
            _isMotorollaByteOrder = false;
            // the 4 bytes after "FUJIFILM" in the makernote point to the start
            // of the makernote
            // IFD, though the offset is relative to the start of the makernote,
            // not the TIFF
            // header (like everywhere else)
            int ifdStart = subdirOffset + get32Bits(subdirOffset + 8);
            processDirectory(_metadata
                    .getDirectory(FujifilmMakernoteDirectory.class),
                    processedDirectoryOffsets, ifdStart, tiffHeaderOffset,offsetSchema);
            _isMotorollaByteOrder = byteOrderBefore;
        } else if (cameraModel != null
                && cameraModel.toUpperCase().startsWith("MINOLTA")) {
            // Cases seen with the model starting with MINOLTA in capitals seem
            // to have a valid Olympus makernote
            // area that commences immediately.
            processDirectory(_metadata
                    .getDirectory(OlympusMakernoteDirectory.class),
                    processedDirectoryOffsets, subdirOffset, tiffHeaderOffset,offsetSchema);
        } else if ("KC".equals(firstTwoChars) || "MINOL".equals(firstFiveChars)
                || "MLY".equals(firstThreeChars)
                || "+M+M+M+M".equals(firstEightChars)) {
            // This Konica data is not understood. Header identified in
            // accordance with information at this site:
            // http://www.ozhiker.com/electronics/pjmt/jpeg_info/minolta_mn.html
            // TODO determine how to process the information described at the
            // above website
            exifDirectory.addError("Unsupported Konica/Minolta data ignored.");
        } else if ("KYOCERA".equals(firstSevenChars)) {
            // http://www.ozhiker.com/electronics/pjmt/jpeg_info/kyocera_mn.html
            processDirectory(_metadata
                    .getDirectory(KyoceraMakernoteDirectory.class),
                    processedDirectoryOffsets, subdirOffset + 22,
                    tiffHeaderOffset,offsetSchema);
        } else if ("Panasonic\u0000\u0000\u0000".equals(new String(_data,
                subdirOffset, 12))) {
            // NON-Standard TIFF IFD Data using Panasonic Tags. There is no
            // Next-IFD pointer after the IFD
            // Offsets are relative to the start of the TIFF header at the
            // beginning of the EXIF segment
            // more information here:
            // http://www.ozhiker.com/electronics/pjmt/jpeg_info/panasonic_mn.html
            processDirectory(_metadata
                    .getDirectory(PanasonicMakernoteDirectory.class),
                    processedDirectoryOffsets, subdirOffset + 12,
                    tiffHeaderOffset,offsetSchema);
        } else if ("AOC\u0000".equals(firstFourChars)) {
            // NON-Standard TIFF IFD Data using Casio Type 2 Tags
            // IFD has no Next-IFD pointer at end of IFD, and
            // Offsets are relative to the start of the current IFD tag, not the
            // TIFF header
            // Observed for:
            // - Pentax ist D
            processDirectory(_metadata
                    .getDirectory(CasioType2MakernoteDirectory.class),
                    processedDirectoryOffsets, subdirOffset + 6, subdirOffset,offsetSchema);
        } else if (cameraModel != null
                && (cameraModel.toUpperCase().startsWith("PENTAX") || cameraModel
                        .toUpperCase().startsWith("ASAHI"))) {
            ...
            // - PENTAX Optio 430
            processDirectory(_metadata
                    .getDirectory(PentaxMakernoteDirectory.class),
                    processedDirectoryOffsets, subdirOffset, subdirOffset,offsetSchema);
        } else {
            // TODO how to store makernote data when it's not from a supported
            // camera model?
            // this is difficult as the starting offset is not known. we could
            // look for it...
    ...
    private void processTag(Directory directory, int tagType,
            int tagValueOffset, int componentCount, int formatCode) {
        ...
        case FMT_UNDEFINED:
            ...
            break;
        case FMT_STRING:
            directory.setString(tagType, readString(tagValueOffset,
                    componentCount));
            break;
        case FMT_SRATIONAL:
        case FMT_URATIONAL:
            if (componentCount == 1) {
                Rational rational = new Rational(get32Bits(tagValueOffset),
                        get32Bits(tagValueOffset + 4));
                directory.setRational(tagType, rational);
            } else {
                Rational[] rationals = new Rational[componentCount];
                for (int i = 0; i < componentCount; i++)
                    rationals[i] = new Rational(get32Bits(tagValueOffset
                            + (8 * i)), get32Bits(tagValueOffset + 4 + (8 * i)));
        ...
        case FMT_BYTE:
            if (componentCount == 1) {
                // this may need to be a byte, but I think casting to int is
                // fine
    ...
    private int calculateTagValueOffset(int byteCount, int dirEntryOffset,
        ...
        if (byteCount > 4) {
            ...
            // dirEntryOffset must be passed, as some makernote implementations
            // (e.g. FujiFilm) incorrectly use an
            // offset relative to the start of the makernote itself, not the
            // TIFF segment.
            final int offsetVal = get32Bits(dirEntryOffset + 8)+offsetSchema;
    ...
    private String readString(int offset, int maxLength) {
        ...
        while ((offset + length) < _data.length
                && _data[offset + length] != '\0' && length < maxLength)
            length++;
    ...
    /**
     * Determine the offset at which a given InteropArray entry begins within
     * the specified IFD.
     * 
     * @param dirStartOffset
