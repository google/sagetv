...
/**
 * A top-level object to hold various types of metadata (such as Exif or IPTC)
 * relating to one entity (such as a file or stream).
 * 
 * Metadata objects may contain zero or more directories. Each directory may
 * contain zero or more tags with corresponding values.
 */
public final class Metadata implements Serializable {
    ...
    /**
     * Returns a <code>Directory</code> of specified type. If this
     * <code>Metadata</code> object already contains such a directory, it is
     * returned. Otherwise a new instance of this directory will be created and
     * stored within this Metadata object.
     * 
     * @param type
    ...
    public Directory getDirectory(Class type) {
        if (!Directory.class.isAssignableFrom(type)) {
            throw new RuntimeException(
                    "Class type passed to getDirectory must be an implementation of sage.media.exif.metadata.Directory");
        }
    ...
    public void dump(PrintStream out) {
        ...
        Iterator directories = getDirectoryIterator();
        while (directories.hasNext()) {
            ...
            if (directory.hasErrors()) {
                ...
                while (errors.hasNext()) {
                    out.println("ERROR in directory: " + directory.getName()
                            + " - " + errors.next());
                }
            }
        }
